...
export default class Analyzer {
   ...
   */
  public processRecipeScanResults (scanResult: string, uri: any): RequestResult['ProcessRecipeScanResults'] {
    if (this.parser === undefined) {
      ...
      symbolArray.forEach((symbol) => {
        if (symbol.kind === SymbolKind.Variable) {
          ...
          if (sameSymbolInScanResultDocument !== undefined) {
            ...
            (processedSymbolInformation[symbol.name] as BitbakeSymbolInformation[]).push(symbol)
          }
        }
      })
    })
